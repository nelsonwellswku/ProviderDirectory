...
namespace Octogami.ProviderDirectory.Application.Domain
{
	...
	{
		private readonly State _defaultState = new State(null, null);

		private readonly Dictionary<string, State> _states =
			new Dictionary<string, State>(StringComparer.InvariantCultureIgnoreCase);
		...
		public InMemoryStateService()
		{
			...
			foreach (var state in stateList)
			{
				...
				_states.Add(state.Abbreviation, state);
			}
		}

		public State GetState(string input)
		{
			return input != null && _states.ContainsKey(input) ? _states[input] : _defaultState;
		}
